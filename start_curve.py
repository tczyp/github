...
'''
        ...
        self.but_curve_type = QComboBox(self.frame_list)
        self.but_curve_type.addItems([u"二维曲线", u"三维曲面"])

        h = QHBoxLayout()
        h.addWidget(self.but_curve_type)
    ...
    def btn_draw(self):
        ...
        try:
            ...
            is_2d = (self.but_curve_type.currentIndex() == 0)

            if is_2d:
                start_str='''
_x = np.linspace(0,1,30)
...
while True:
    i += 1
    fun = locals().get('sf' + str(i), None)
    if fun:
        _z_list.append(map(fun, _x, _y))
    else:
        break

self.clearp()
for _z in _z_list:
    ax_3d = self.sc.fig.gca(projection='3d')
    ax_3d.plot_surface(_x, _y, _z, rstride=1, cstride=1, cmap=plt.get_cmap('rainbow'))
'''

            exe_str = unicode(cin + start_str + ui + end_str)
            exec (exe_str)
